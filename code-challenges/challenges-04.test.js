...
const generateSubmitButton = () => {
  let newButton = $('<button></button>').text('submit');
...
const isCapitalized = (str) => {
  let capWord = [];
  let checkCap = /\b[A-Z][a-z]+/g;
  capWord = str.match(checkCap);
  if (str.match(checkCap) === null) {
    capWord = [];
  }
  return capWord;
};
...
const citiesAtoJ = (arr) => {
  ...
  arr.forEach(city => {
    ...
    let check = city.match(regex);
    if (check !== null) {
      citiesAJ.push(check[0]);
    }
  });
  return citiesAJ;
};
